...
class FasterWhisperSTTProvider(BufferedSTTProvider):
    ...
    def transcribe_pcm(self, pcm: bytes) -> str:
        ...
        segments, _info = self._ensure_model().transcribe(
            ...
            # Corta silêncio antes de transcrever (menos alucinação, mais
            # rápido). `speech_pad_ms` generoso e `min_silence_duration_ms`
            # alto de propósito: o item 6 da v1.3 exige não cortar a primeira
            # nem a última palavra, e uma pausa curta no meio de "Opa, tudo
            # bem?" não pode ser tratada como fim de fala.
            vad_filter=True,
