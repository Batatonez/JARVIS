...
- O projeto está atualmente no **JARVIS Backend v0.4**: uma Application Layer (`JarvisApplication`, `app/application.py`) estável entre o Core e qualquer frontend (terminal hoje; HUD/voz no futuro), sobre o mesmo núcleo assíncrono com a arquitetura do Claude Agent SDK pronta (`ClaudeAgentProvider`) e conectável via `ANTHROPIC_API_KEY`. Um frontend novo deve falar com `JarvisApplication` — nunca com `JarvisCore`, `Orchestrator`, `MemoryService` ou `services/claude_agent_provider.py` diretamente (ver [`docs/application-api.md`](docs/application-api.md)). Muitas pastas ainda existem apenas como estrutura documentada para crescimento futuro — isso é intencional, não um esquecimento.
- Este arquivo (`CLAUDE.md`) orienta o Claude Code **desenvolvendo** o JARVIS. As instruções de persona do JARVIS em runtime (o que o assistente sabe sobre si mesmo numa conversa com o usuário) ficam em [`services/runtime_identity.py`](services/runtime_identity.py) — são coisas diferentes, não duplique conteúdo entre os dois.
...
**Histórico de conversa não é memória.** Desde a v0.4, `app/conversation.py` mantém o histórico de mensagens da sessão atual em RAM (`JarvisApplication.get_messages()`) — isso é diferente de `memory/`. Nunca copiar mensagens de conversa para `memory/profile.md`/`preferences.md` automaticamente; isso continua exigindo o mesmo critério de utilidade futura clara mencionado acima, e deve ser uma ação deliberada, não automática.
...
O JARVIS está no **JARVIS Backend v0.4**: `JarvisApplication` (`app/application.py`) é a fronteira estável entre o Core e qualquer frontend — histórico de conversa em runtime, stream de eventos, status consolidado, cancelamento de requisição e política de concorrência (uma requisição ativa por conversa). Por baixo, continua o mesmo núcleo assíncrono (`python main.py`) com orquestração, comandos internos, leitura somente-leitura de memória, um event bus interno e a arquitetura do **Claude Agent SDK** pronta via `ClaudeAgentProvider` (`services/claude_agent_provider.py`, pacote `claude-agent-sdk` — única dependência externa do projeto). Sem `ANTHROPIC_API_KEY` configurada, o sistema cai automaticamente para o placeholder `UnavailableAIService`, sem erro — e é esse o comportamento padrão no ambiente de desenvolvimento atual, não uma falha. Nenhum framework de interface gráfica foi escolhido, e nenhuma voz, automação, MCP, ferramentas reais, persistência de conversa ou Ruflo/multiagentes foram implementados ainda. Ver [`docs/architecture.md`](docs/architecture.md) para o detalhamento completo de implementado vs. preparado (mas não ativado) vs. planejado, e [`docs/application-api.md`](docs/application-api.md) para a API pública da Application Layer. Ao trabalhar em tarefas futuras, não presuma que uma camada existe só porque está no diagrama de arquitetura — verifique o estado real do código antes de assumir.

**Nunca peça, solicite ou tente obter uma `ANTHROPIC_API_KEY` (ou qualquer outra credencial) do usuário durante o desenvolvimento**, a menos que ele mesmo traga o assunto. Trabalhar sem IA real conectada é o estado normal e esperado deste projeto — valide integrações de IA com mocks/fakes, nunca com chamadas reais à API.
