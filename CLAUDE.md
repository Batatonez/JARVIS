...
- O projeto está atualmente no **JARVIS Core v0.3**: um núcleo executável por terminal, assíncrono, com a arquitetura do Claude Agent SDK pronta (`ClaudeAgentProvider`) e conectável via `ANTHROPIC_API_KEY`. Muitas pastas ainda existem apenas como estrutura documentada para crescimento futuro — isso é intencional, não um esquecimento.
...
O JARVIS está no **JARVIS Core v0.3**: um núcleo executável por terminal (`python main.py`), assíncrono, com orquestração básica, comandos internos, leitura somente-leitura de memória, um event bus interno e a arquitetura do **Claude Agent SDK** pronta via `ClaudeAgentProvider` (`services/claude_agent_provider.py`, pacote `claude-agent-sdk` — única dependência externa do projeto). Sem `ANTHROPIC_API_KEY` configurada, o sistema cai automaticamente para o placeholder `UnavailableAIService`, sem erro — e é esse o comportamento padrão no ambiente de desenvolvimento atual, não uma falha. Nenhum framework de interface gráfica foi escolhido, e nenhuma voz, automação, MCP, ferramentas do agente, memória conversacional persistente ou Ruflo/multiagentes foram implementados ainda. Ver [`docs/architecture.md`](docs/architecture.md) para o detalhamento completo de implementado vs. preparado (mas não ativado) vs. planejado. Ao trabalhar em tarefas futuras, não presuma que uma camada existe só porque está no diagrama de arquitetura — verifique o estado real do código antes de assumir.
