...
✅ **JARVIS v1.8 — Files, Search & Skills.** O JARVIS passou a encontrar arquivos de verdade: `Ctrl+K` e "acha meu trabalho de história" devolvem o arquivo, com busca por nome (exata, parcial, por palavras soltas e difusa — "parasita" acha `PARASITAS E DOENÇAS.pdf`), por metadata ("PDFs de ontem", "imagens desta semana") e **por conteúdo** ("o arquivo que fala de manguezal") via FTS5 do próprio SQLite. Acento e caixa não importam. Tudo isso é **local**: nenhum caminho, metadata ou conteúdo sai da máquina, e a busca funciona com todos os providers de IA fora do ar — só o RESUMO envolve nuvem, e só quando o usuário clica em "Resumir". O índice cobre Documentos, Área de Trabalho e Downloads por padrão, é incremental e **nunca** indexa `.env`, chave privada, `node_modules`, `.git`, `.claude` ou o próprio banco do JARVIS — nem dentro de uma pasta que o usuário adicionou. As ações de arquivo usam um **handle opaco** (`file_2`) em vez de caminho, o que elimina injeção de caminho em uso de ferramenta. Por baixo, uma arquitetura de [Skills](app/skills.py) com schema, permissão e nível de risco declarados coordena Calculadora, Sistema e Arquivos sem reimplementar nenhum serviço — e sem nenhuma skill de shell arbitrário. A Command Bar ganhou navegação de teclado completa: Tab, Shift+Tab e setas percorrem os resultados com wrap, Enter executa o selecionado (ou a query, se nada estiver selecionado), e o mouse compartilha o mesmo estado de seleção.

✅ **JARVIS v1.7.0 — Universal Command Bar & System Control.** `Ctrl+K` abre uma barra de comando que entende o que foi pedido antes de decidir o que fazer com aquilo: abrir um aplicativo, mexer no volume, tirar um print, ler a área de transferência, listar processos ou fazer uma conta são resolvidos **localmente**, sem chamada de rede — e continuam funcionando com todos os providers de IA fora do ar. O que não é um comando claro segue para o chat normalmente; na dúvida, é conversa, porque um comando virar conversa é inofensivo e o contrário abre programa sem pedido. Entender e executar são etapas separadas por construção ([`app/intents.py`](app/intents.py) só classifica; [`app/actions.py`](app/actions.py) autoriza): toda ação carrega nível de risco e categoria de permissão, e ação de risco alto — fechar um programa, que pode descartar trabalho não salvo — **sempre** exige confirmação explícita, inclusive quando proposta pelo modelo de IA. Aplicativos são descobertos pelo Menu Iniciar do próprio Windows com índice reusado, em vez de uma lista fixa ou de varredura do disco. `Ctrl+Space` continua sendo o microfone.
